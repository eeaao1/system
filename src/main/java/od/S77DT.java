...
public class S77DT {
    ...
    public static void main(String[] args) {
        int[] yy = {8, 2, 10, 5, 7};
        System.out.println(p(1, 3, yy) + 10);
    }

    public static int p(int l, int r, int[] piazz) {
        if (l > r) {
            return 0;
        }
        if (piazz[l] > piazz[r]) {
        ...
        int tl = l == 0 ? piazz.length - 1 : l - 1;
        int tr = r == piazz.length - 1 ? 0 : r + 1;
        int p1 = p(tl, r, piazz);
        int p2 = p(l, tr, piazz);
        return Math.max(p1+piazz[l], p2+piazz[r]);
    }
